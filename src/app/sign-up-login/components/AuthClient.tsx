    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="hidden lg:flex lg:w-[45%] xl:w-[40%] bg-surface border-r border-border flex-col justify-between p-12 relative overflow-hidden">
        <div
          ...
          style={{ background: 'radial-gradient(ellipse 80% 60% at 30% 40%, rgba(0,229,160,0.06) 0%, transparent 70%)' }}
        />
        <div
        ...
        <div className="relative z-10">
          <Link href="/" className="flex items-center gap-3 mb-16">
            <AppLogo size={40} />
            <span className="font-display font-bold text-2xl tracking-tight text-foreground">VoltEdge</span>
          </Link>
        ...
        <div className="relative z-10 space-y-4">
          {[
            { icon: 'ShieldCheckIcon', title: 'Official Warranty', desc: 'All products carry manufacturer warranty' },
            { icon: 'CreditCardIcon', title: '0% EMI Available', desc: 'Up to 24 months on all banks' },
            { icon: 'TruckIcon', title: 'Nationwide Delivery', desc: 'Fast delivery to all 64 districts' },
          ].map((item) => (
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground text-sm">
              {mode === 'login' ? 'Sign in to your VoltEdge account' :
               mode === 'register' ? 'Join VoltEdge for exclusive deals and EMI access' :
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {mode !== 'forgot' && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {mode === 'login' && (
                    <button type="button" onClick={() => setMode('forgot')} className="text-xs text-accent hover:text-accent/80 transition-colors">
                      Forgot password?
                    </button>
                  )}
                </div>
            ...
            {mode === 'register' && (
              <div>
                <label className="block text-xs font-medium text-muted-foreground uppercase tracking-widest mb-2">Confirm Password</label>
                <div className="relative">
                  <input
          ...
          <div className="mt-6 text-center text-sm text-muted-foreground">
            {mode === 'login' ? (
              <>Don&apos;t have an account?{' '}
                <button onClick={() => { setMode('register'); setError(''); setSuccess(''); }} className="text-accent hover:text-accent/80 font-semibold transition-colors">
                  Sign up free
                </button>
              </>
            ) : mode === 'register' ? (
              <>Already have an account?{' '}
                <button onClick={() => { setMode('login'); setError(''); setSuccess(''); }} className="text-accent hover:text-accent/80 font-semibold transition-colors">
                  Sign in
                </button>
              </>
            ) : (
          ...
          <div className="mt-6 p-3 rounded-xl bg-elevated border border-border">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-semibold text-foreground">Secure JWT Authentication</span>
            </div>
            <p className="text-xs text-muted-foreground">Your session is secured with JWT tokens. Tokens are automatically refreshed and stored securely.</p>
          </div>
        </div>
      </div>
    </div>
