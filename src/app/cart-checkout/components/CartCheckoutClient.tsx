        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <React.Fragment key={s.key}>
              <div
                className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${isActive ? 'bg-accent/10 border border-accent/30' : ''}`}
              >
                <div
                ...
                <span
                  className={`text-sm font-medium font-display ${isActive ? 'text-foreground' : 'text-muted-foreground'}`}
                >
                  {s.label}
                </span>
              </div>
              {i < STEPS.length - 1 && (
                <div
                  className={`w-12 h-px mx-1 ${stepKeys.indexOf(s.key as Step) < currentIdx ? 'bg-accent' : 'bg-border'}`}
                />
              )}
            </React.Fragment>
          );
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-muted-foreground hover:text-danger transition-colors"
                ...
                {[
                  ...
                  <div key={field.key} className={field.key === 'street' ? 'sm:col-span-2' : ''}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAddress((prev) => ({ ...prev, [field.key]: e.target.value }))
                      }
                  ...
                  {[
                    {
                      ...
                      price: 0,
                      condition: 'Free on orders above ৳50,000',
                    },
                    {
                      id: 'express',
                    ...
                    <label
                      key={opt.id}
                      className="flex items-center gap-4 p-4 bg-surface border border-border rounded-2xl cursor-pointer hover:border-accent/30 transition-all"
                    >
                      <input
                        type="radio"
                        name="delivery"
              ...
              <div className="space-y-3">
                {[
                  ...
                  },
                  {
                    id: 'emi',
                    label: '0% EMI',
                    desc: 'Up to 24 months on major banks',
                    icon: 'CalendarIcon',
                  },
                ].map((method) => (
                  <label
                  ...
                  >
                    ...
                    />
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${paymentMethod === method.id ? 'bg-accent/10 border border-accent/30' : 'bg-elevated border border-border'}`}
                    >
                      <Icon
                        name={method.icon as 'CreditCardIcon'}
                        size={18}
                        className={
